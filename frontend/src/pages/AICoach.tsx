  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 flex flex-col pt-16 lg:pt-0">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden z-0">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full bg-emerald-700/5 blur-3xl" />
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative z-10 max-w-7xl mx-auto w-full p-4 lg:p-8 gap-6 h-[100dvh]">
          ...
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 shadow-lg backdrop-blur-md">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Your sustainability guide</p>
              </div>
            </div>
              ...
              <div>
                ...
                {contextLoading ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center bg-slate-900/50 border border-slate-700/40 p-3 rounded-xl">
                      <span className="text-sm text-slate-400">Carbon Score</span>
                      <span className="text-lg font-bold text-emerald-400">{baseline.carbon_score.toFixed(0)}</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-900/50 border border-slate-700/40 p-3 rounded-xl">
                      <span className="text-sm text-slate-400">Top Source</span>
                      <div className="text-right">
                        <p className="text-sm font-bold text-rose-400">{getTopSource(baseline).name}</p>
                        <p className="text-xs text-slate-500">{getTopSource(baseline).val.toFixed(1)} kg</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="bg-slate-900/50 border border-slate-700/40 p-4 rounded-xl text-center">
                    <p className="text-sm text-slate-400 mb-2">No assessment found.</p>
                    <button
        ...
        <div className="flex-1 flex flex-col bg-slate-800/40 border border-slate-700/50 rounded-2xl shadow-xl backdrop-blur-md overflow-hidden h-full min-h-0">
          
          {/* Chat Header */}
          <div className="px-6 py-4 border-b border-slate-700/50 bg-slate-800/60 flex justify-between items-center shrink-0">
            <div>
              <h2 className="text-sm font-semibold text-slate-200">Conversation</h2>
              <p className="text-xs text-slate-500">Ask me anything about your footprint or habits</p>
            </div>
            {isTyping && (
              <span className="text-xs font-medium text-emerald-400 animate-pulse bg-emerald-500/10 px-2 py-1 rounded-md border border-emerald-500/20">
                Coach is typing...
              </span>
            )}
          </div>
