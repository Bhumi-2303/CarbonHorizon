...
function ErrorBanner({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center gap-4 py-16 text-center">
      ...
      <div>
        <p className="text-muted font-medium">Failed to load history</p>
...
function EmptyState({ onRecalculate }: { onRecalculate: () => void }) {
  return (
    <div className="flex flex-col items-center gap-5 py-20 text-center">
      ...
      <div>
        <h3 className="text-lg font-semibold text-primary">No assessments yet</h3>
      ...
      <button
        ...
        className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold text-primary
...
function AssessmentCard({
  ...
  return (
    <button
      ...
      type="button"
      onClick={onClick}
      className="w-full text-left group bg-white/5 hover:glass-panel border border-slate-700/50
    ...
    >
      <div className="flex items-start gap-4">
        {/* Rank pill */}
        <div className="flex-shrink-0 w-8 h-8 rounded-full border-white/20/60 flex items-center justify-center">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              ...
              <span className="inline-block mt-1 text-xs text-muted border-white/20/50 px-2 py-0.5 rounded-full">
          ...
          <div className="mt-3 flex items-center justify-between gap-4">
            ...
            <div className="hidden sm:flex flex-col gap-1 min-w-[140px]">
              {[
                { label: <Car className="w-4 h-4 inline-block text-accent" />, val: item.transport, total: item.total_emission },
                { label: <Zap className="w-4 h-4 inline-block text-accent" />, val: item.energy,    total: item.total_emission },
                { label: <Salad className="w-4 h-4 inline-block text-accent" />, val: item.food,      total: item.total_emission },
                { label: <Trash2 className="w-4 h-4 inline-block text-accent" />, val: item.waste,     total: item.total_emission },
              ].map(({ label, val, total }, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <span className="text-xs w-5 text-center">{label}</span>
                  <div className="flex-1 h-1.5 border-white/20/80 rounded-full overflow-hidden">
                    <div
          ...
          {/* Score gauge bar */}
          <div className="mt-3">
            <div className="h-1 border-white/20/60 rounded-full overflow-hidden">
              <div
        ...
        <div className="flex items-center justify-between gap-4 mb-8">
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-primary">Assessment History</h1>
          ...
          <button
            ...
            className="flex-shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold
              text-primary bg-gradient-to-r from-emerald-500 to-emerald-600
        ...
        {status === 'success' && items.length > 0 && (
          <div className="grid grid-cols-3 gap-3 mb-6 animate-fade-in">
            {[
              ...
              <div
                key={label}
                className="glass-panel border border-slate-700/50 rounded-2xl p-4 text-center"
              >
