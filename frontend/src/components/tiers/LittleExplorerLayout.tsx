...
export default function LittleExplorerLayout() {
  ...
  return (
    <div className="min-h-screen bg-bg-primary flex flex-col font-sans">
      <header className="fixed top-4 left-4 right-4 z-50 flex items-center justify-between glass-card px-4 py-2 border-emerald-500/20">
        ...
        <nav className="flex-1 px-4 flex items-center gap-2">
          <NavLink 
            to="/dashboard" 
            className={({ isActive }) => `px-4 py-2 rounded-xl font-bold transition-all ${isActive ? 'bg-emerald-400/20 text-emerald-400' : 'text-slate-400 hover:text-primary'}`}
          >
            My Dashboard
          </NavLink>
          <NavLink 
        ...
        <div className="flex items-center gap-4 pl-6 border-l border-slate-700/50">
          <p className="text-sm font-bold text-emerald-400 hidden sm:block">
